...
- **D-04-REVISED (2026-04-19, UAT gap closure):** Bump to **Tailwind v4.x standalone CLI** (still no Node runtime — v4 ships a standalone binary at `github.com/tailwindlabs/tailwindcss/releases`). Reason: the vendored `basecoat.css@0.3.3` is a Tailwind-v4 library — uses `@custom-variant` (line 1), `@theme { ... }` (line 86), `@apply @container/*` and `has-data-[slot=...]` arbitrary variants (line 430). v3.4.19's CLI cannot parse these. Plan 02 shipped a build that silently dropped Basecoat entirely; UAT Gap #1 surfaced this. v4 also resolves `@import` natively, so it fixes the original `@import`-not-processed symptom in the same move. `_compat.css` stays (v4 supports `@apply`). `input.css` updates syntax: `@import "tailwindcss";` replaces the three `@tailwind` directives. `tailwind.config.js` either kept via v4's `@config` directive or migrated to CSS `@theme` (planner's call). D-03 and D-07 preserved unchanged.
